...
function showAbout(){
  showDialog(`<div class="dialog-kicker">${icon('graph')} WHY MOVEGRAPH</div><h2 id="dialog-title">Seven cities taught me to stop treating a move like a checklist.</h2><p class="dialog-intro">A list shows what is due. A dependency map shows what will move, what will stay put, and where a person still needs to decide.</p><div class="about-facts"><p><strong>Personal starting point.</strong> MoveGraph comes from Hayk’s experience living in seven cities over four years. The Tokyo-to-San Francisco dates and notes are an illustrative scenario, not official relocation requirements.</p><p><strong>Interaction rule.</strong> Edit any flexible step directly. Dates flow forward to dependent steps, never backward. A date set by you becomes a boundary; if an upstream delay makes it impossible, MoveGraph preserves your intent and surfaces the conflict.</p><p><strong>Design judgment.</strong> Fixed commitments never move silently. AI-suggested links remain inactive until a person accepts them, and every applied plan can be undone.</p><p><strong>How AI contributed.</strong> AI helped structure the sample scenario, surface edge cases, and critique the interaction. The dates shown here come from visible local rules; no live model or external booking is connected.</p></div><button class="button button-primary" id="about-done">Back to the plan ${icon('arrow')}</button>`);
